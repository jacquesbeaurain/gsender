...
(async () => {
    ...
    await scenario('grbl handshake', [], 2501, async (c) => {
        ...
        check(
            'status report parses as grbl 1.1',
            /^<Idle\|MPos:[-0-9.,]+\|Bf:\d+,\d+/.test(status),
            status,
        );
    });
    ...
    await scenario(
        ...
        async (c) => {
            ...
            check(
                'AXS reports 4 axes with letters',
                axs === '[AXS:4:XYZA]',
                axs,
            );
            c.clear();
            ...
            const status = c.last((l) => l.startsWith('<'));
            check(
                '4-axis MPos has 4 values',
                status.match(/MPos:([-0-9.,]+)/)[1].split(',').length === 4,
                status,
            );
            c.clear();
            ...
            await wait(150);
            check(
                '0x87 returns a complete report with WCO',
                /WCO:/.test(c.last((l) => l.startsWith('<')) || ''),
            );
        },
    );
    ...
    await scenario(
        ...
        async (c) => {
            ...
            check(
                'state is Alarm',
                /^<Alarm\|/.test(c.last((l) => l.startsWith('<')) || ''),
            );
            ...
            await wait(200);
            check(
                'G-code rejected while alarm-locked (error:9)',
                c.lines.includes('error:9'),
                c.lines.join(' | '),
            );
            ...
            check(
                ...
                /^<Idle\|/.test(c.last((l) => l.startsWith('<')) || ''),
            );
        },
    );
    ...
    await scenario('homing defers ok until complete', [], 2504, async (c) => {
        ...
        check(
            'state is Home',
            /^<Home\|/.test(c.last((l) => l.startsWith('<')) || ''),
        );
        await wait(2800);
        check(
            'ok arrives after homing completes',
            c.lines.includes('ok'),
            c.lines.join(' | '),
        );
        c.clear();
        ...
        const status = c.last((l) => l.startsWith('<'));
        check(
            'homed to -pulloff (-2.000)',
            /MPos:-2\.000,-2\.000,-2\.000/.test(status),
            status,
        );
    });
    ...
    await scenario('probe + zero workflow', [], 2505, async (c) => {
        ...
        check(
            'PRB reported with success flag 1',
            prb === '[PRB:0.000,0.000,-10.000:1]',
            prb,
        );
        check(
            'probe move got exactly 3 oks',
            c.lines.filter((l) => l === 'ok').length === 3,
            String(c.lines.filter((l) => l === 'ok').length),
        );
        ...
        check(
            'G54 Z offset set from probe stop position',
            g54 === '[G54:0.000,0.000,-10.000]',
            g54,
        );
        c.clear();
        c.send('?');
        ...
        check(
            'WCO reflects the new offset',
            /WCO:0\.000,0\.000,-10\.000/.test(status || ''),
            status,
        );
    });
    ...
    await scenario(
        ...
        async (c) => {
            ...
            await wait(2500);
            check(
                'ALARM:5 on no contact',
                c.lines.includes('ALARM:5'),
                c.lines.join(' | '),
            );
            const prb = c.last((l) => l.startsWith('[PRB:'));
    ...
    // ---------------------------------------------------------------
    await scenario(
        'planner back-pressure + ok accounting',
        [],
        2507,
        async (c) => {
            await wait(500);
            ...
            check(
                'planner throttles: fewer than 40 oks immediately',
                early < 40 && early >= 14,
                String(early),
            );
            c.send('?');
            ...
            const status = c.last((l) => l.startsWith('<'));
            check(
                'Bf shows a full planner (0 free)',
                /Bf:0,/.test(status),
                status,
            );
            // let it drain
    ...
    await scenario('feed hold / resume / soft reset', [], 2508, async (c) => {
        ...
        check(
            'feed hold gives Hold:1 while a block is live',
            /^<Hold:1\|/.test(held),
            held,
        );
        ...
        check(
            'position frozen during hold',
            Math.abs(posLater - posAtHold) < 0.001,
            `${posAtHold} -> ${posLater}`,
        );
        check(
            'hold settles to Hold:0 once decelerated',
            /^<Hold:0\|/.test(stillHeld),
            stillHeld,
        );
        ...
        await wait(150);
        check(
            'cycle start resumes Run',
            /^<Run\|/.test(c.last((l) => l.startsWith('<')) || ''),
        );
        ...
        await wait(400);
        check(
            'soft reset re-emits banner',
            c.lines.some((l) => /^Grbl 1\.1f/.test(l)),
            c.lines.join(' | '),
        );
        c.send('?');
        await wait(150);
        check(
            'soft reset stops motion (Idle)',
            /^<Idle\|/.test(c.last((l) => l.startsWith('<')) || ''),
        );
    });
    ...
    await scenario('M0 pause parks in Hold', [], 2509, async (c) => {
        ...
        check(
            'X stopped at 5 (line after M0 not run)',
            /MPos:5\.000/.test(status),
            status,
        );
        c.send('~');
        await wait(600);
        ...
        await wait(150);
        check(
            'cycle start continues past M0',
            /MPos:(10\.000|[6-9]\.)/.test(
    ...
    await scenario(
        ...
        async (c) => {
            ...
            await wait(600);
            check(
                'every 3rd line errors',
                c.lines.filter((l) => l === 'error:20').length >= 1,
                c.lines.join(' | '),
            );
            check(
                'errors replace ok, never accompany it',
                c.lines.filter((l) => l === 'ok').length +
                    c.lines.filter((l) => l.startsWith('error:')).length ===
                    4,
                c.lines.join(' | '),
            );
        },
    ...
    // ---------------------------------------------------------------
    await scenario(
        'no-banner exercises $I fallback',
        ['--no-banner'],
        2511,
        async (c) => {
            await wait(600);
    ...
    await scenario('G91 relative + G20 inches', [], 2512, async (c) => {
        ...
        await wait(150);
        check(
            'G91 move is relative (X=15)',
            /MPos:15\.000,10\.000/.test(c.last((l) => l.startsWith('<')) || ''),
            c.last((l) => l.startsWith('<')),
        );
        ...
        await wait(150);
        check(
            'G20 inch move converts (X1in = 25.4mm)',
            /MPos:25\.400/.test(c.last((l) => l.startsWith('<')) || ''),
            c.last((l) => l.startsWith('<')),
        );
    });
    ...
    // ---------------------------------------------------------------
    await scenario(
        '$J= jogging (the form GrblController sends)',
        [],
        2513,
        async (c) => {
            await wait(500);
            ...
            await wait(300);
            check(
                '$J= is accepted, not error:3',
                c.lines.includes('ok') &&
                    !c.lines.some((l) => l.startsWith('error:')),
                c.lines.join(' | '),
            );
            await wait(600);
            c.clear();
            ...
            check(
                'jog moved X by 5mm',
                /MPos:5\.000/.test(c.last((l) => l.startsWith('<')) || ''),
                c.last((l) => l.startsWith('<')),
            );
            ...
            await wait(150);
            check(
                'state is Jog while jogging',
                /^<Jog\|/.test(c.last((l) => l.startsWith('<')) || ''),
                c.last((l) => l.startsWith('<')),
            );
            ...
            check(
                '0x85 cancels the jog back to Idle',
                /^<Idle\|/.test(after),
                after,
            );
            const x = Number(after.match(/MPos:([-0-9.]+)/)[1]);
            check(
                'jog cancel stopped short of the 105mm target',
                x > 5 && x < 105,
                String(x),
            );
            ...
            await wait(250);
            check(
                'jog without F is rejected (error:22)',
                c.lines.includes('error:22'),
                c.lines.join(' | '),
            );
        },
    );
    ...
    // ---------------------------------------------------------------
    await scenario(
        'jog rejected while alarm-locked',
        ['--alarm-on-connect=1'],
        2514,
        async (c) => {
            await wait(700);
